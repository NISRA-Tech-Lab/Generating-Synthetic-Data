...
if( exists("anonymisation_data") ){
  ...
  anon_cols <- colnames(anonymisation_data)
  ...
  for (x in 1:(ncol(df))) {
    ...
    if (data_type == "character") {
      ...
      character_data <- anonymisation_data %>%
        select(x)
      char_col <- colnames(character_data)
      ...
      synthetic_df[, col_numeric] <- anonymised_postcode
...
if( exists("synthetic_df") ){
  anonymised_names <- colnames(synthetic_df) 
  synthetic_data_output <- synthetic_data_output %>%
